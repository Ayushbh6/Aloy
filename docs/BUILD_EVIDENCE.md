...
The close/reopen crash was traced to AppKit releasing the panel window. The shell now keeps that window allocated; two red-close/orb-reopen cycles returned to Ready without a new crash, with one shell and one backend process running. Aloy still requires manual launch after a crash or Mac restart.

...
SQLite migrations own conversations, ordered messages, runs, audio metadata, provider sessions, settings and a cost ledger independent of deletable conversations. Audio lives as private files in macOS Application Support. The Swift/AppKit shell speaks JSON-lines v1 to the Python bridge; it contains no model logic. Default Standard mode is microphone to local Silero activity detection and leading/trailing silence trim to Qwen ASR 1.7B to `ChatAgent` to sentence-level Chatterbox Multilingual V3 TTS. Chatterbox is the only installed offline TTS voice. Optional paid speech routes are direct Gemini 3.8 Flash-Lite/Flash TTS and OpenRouter Gemini 3.8 Flash-Lite/Flash or Grok Voice TTS, selected explicitly in the same picker and run through the same output, persistence, cancellation and estimated spending path. Stop cancels the active bridge task and clears local playback.
